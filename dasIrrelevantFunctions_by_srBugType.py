...
# these did not initiate the memory allocation. They are not used to determine the location of the function and/or
# calculate the stack hash.

asExceptionHelperFunctions = [
  # Functions that are widely used to raise an exception:
...
asBreakpointHelperFunctions = [
  # Functions that are widely used to trigger a breakpoint:
...
dasIrrelevantFunctions_by_srBugType = {
  "AppExit": [
  ...
  "Assert": [
    # Assertions are commonly reported by triggering a breakpoint
  ] + asBreakpointHelperFunctions + [
    # Edge specific
  ...
  r"C\+\+(:.*)?": asExceptionHelperFunctions, # Functions that are widely used to raise an exception
  "CFG": asCFGFunction,
  "CorruptList": [
    "msvcrt.dll!malloc",
  ...
  "InvalidHandle": asExceptionHelperFunctions, # Functions that are widely used to raise an exception
  "OOM": [
    # OOMs are often reported by triggering a breakpoint:
  ] + asBreakpointHelperFunctions + [
    # Functions that are widely used to report OOM
    "kernelbase.dll!RaiseFailFastException",
    "kernelbase.dll!TerminateProcessOnMemoryExhaustion",
    "iertutil.dll!ATL::AtlThrowImpl",
    # Functions that are widely used to allocate memory and their helpder functions:
