...
def fs0ProcessOutputOrThrowExceptionIfItContainsJunk(uAddress, asbSymbolOutput):
  ...
  if fu0ValueFromCdbHexOutput(asbSymbolOutput[0]) == uAddress:
    return None;
...
def cProcess_fsb0GetSymbolForAddress(oProcess, uAddress):
  ...
  try:
    ...
    return fs0ProcessOutputOrThrowExceptionIfItContainsJunk(uAddress, asbSymbolOutput);
  except:
    pass;
  asbSymbolOutput = oProcess.fasbExecuteCdbCommand(
    sbCommand = sbCommand, 
  ...
  try:
    ...
    raise AssertionError(
      "Unexpected get symbol output (%s):\r\n%s" % (
