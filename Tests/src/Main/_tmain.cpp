// disable warnings about _alloca potentially throwning an exception
#pragma warning( disable : 6255 )
// disable warnings about _alloca use inside a loop
...
extern "C" {
  ...
  VOID __stdcall fIntegerOverflow();
...
/*
* This has been disabled as I do not know a way to support Unicode module names in windbg
* because output through stdout is limited to bytes. It also appears to interfere with
* ASan support, but I suspect that can be fixed by compiling the DLL differently.
// Load a library with Unicode characters in its name to test handling of such names.
#ifdef _WIN64
#ifdef _DEBUG
  LoadLibraryW(L"UnicodeDLL😀_x64d");
#else 
  LoadLibraryW(L"UnicodeDLL😀_x64");
#endif
#else 
#ifdef _DEBUG
  LoadLibraryW(L"UnicodeDLL😀_x86d");
#else 
  LoadLibraryW(L"UnicodeDLL😀_x86");
#endif
#endif
*/
  // disable buffering
  ...
  if (
    ...
    hResult = WindowsCreateStringReference(asArguments[3], (UINT32)_tcslen(asArguments[3]), &hStringHeader, &hString);
    if (!SUCCEEDED(hResult)) {
    ...
    if (hSecondHeap == 0) {
      _ftprintf(stderr, _T("Cannot create a second heap.\r\n"));
      return 1;
    };
    ...
    BOOL bSignedness = !bIncrease && !bDecrease && !bMultiply && !bTruncate && _tcsicmp(asArguments[2], _T("signedness")) == 0;
    if (!bIncrease && !bDecrease && !bMultiply && !bTruncate && !bSignedness) {
    ...
    BOOL bUnsigned = _tcsicmp(asArguments[3], _T("unsigned")) == 0;
